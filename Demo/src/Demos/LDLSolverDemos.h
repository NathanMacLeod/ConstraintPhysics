...
public:
	...
	std::map<std::string, std::string> askParameters() override {
		...
		out["which_demo"] = pickParameterFromOptions(
			"Choose from one of the demos that demonstrate the advantages of the LDL-PGS solver: 1) Scissor lift, 2) High mass ratio:  ", { "1", "2"}
		);
	...
	void run() override {
		...
		if (parameters["use_ldl"] == "2") {
			p.setPGSIterations(0, 0, 30);
		}
		else {
			p.setPGSIterations(35, 15, 0);
		}

		bool using_scissor_lift = parameters["which_demo"] == "1";
		...
		if (using_scissor_lift) {
			...
			for (int i = 0; i < n_chain; i++) {
				...
				if (i == 0) {
					p.addBallSocketConstraint(r2, this_chain_r, this_chain_pos);
				}
				else {
			...
			ball_r->setVel(mthz::Vec3(0, 30, 9));
		...
		while (rndr::render_loop(&fElapsedTime)) {
			...
			if (using_scissor_lift && rndr::getKeyDown(GLFW_KEY_I)) {
			...
			else if (using_scissor_lift && rndr::getKeyDown(GLFW_KEY_K)) {
				double current_dist = p.getDistanceConstraintTargetDistance(scissor_height_right);
