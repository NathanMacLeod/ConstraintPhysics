...
class TestBasicCapsuleAgainstGround : public Test {
public:
	std::string getTestName() const override { return "Capsule vs Ground"; }
	bool canBeRunWithGraphics() const override { return true; }
	TestExpectationStatus getTestExpectation() const override { return TestExpectationStatus::REQUIRED; }
	phyz::PhysicsEngine* initTest(uint32_t n_threads, std::vector<PhysBod>* bodies) override {
		collision_occured = false;
		...
		//create capsule
		double capsule_radius = 1.0;
		double capsule_drum_height = 1.0;
		phyz::ConvexUnionGeometry capsule_geom = phyz::ConvexUnionGeometry::capsule(mthz::Vec3(0, 5.0, 0), capsule_radius, capsule_drum_height);
		box_r = p->createRigidBody(capsule_geom);
		bodies->push_back(PhysBod{ fromGeometry(capsule_geom), box_r });
...
public:
	...
	std::vector<std::unique_ptr<Test>> getTests() const override {
		...
		out.push_back(std::make_unique<TestBasicCapsuleAgainstGround>());
		return out;
	}
};